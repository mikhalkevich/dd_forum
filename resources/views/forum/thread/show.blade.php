...
@section ('content')
    <div id="thread" class="v-thread">
        <div class="flex flex-col md:flex-row justify-between my-4">
            <h2 class="grow text-3xl text-gray-50 font-semibold">{{ $thread->title }}</h2>

            <div class="flex items-center gap-2">
                @if (Gate::allows('deleteThreads', $thread->category) && Gate::allows('delete', $thread))
                    @if ($thread->trashed())
                        <x-forum.button-link href="#" class="bg-red-500 text-white" data-open-modal="perma-delete-thread">
                            <i data-feather="trash"></i> {{ trans('forum::general.perma_delete') }}
                        </x-forum.button-link>
                    @else
                        <x-forum.button-link href="#" class="bg-red-500 text-white inline-flex items-center gap-2" data-open-modal="delete-thread">
        ...
        @if ((count($posts) > 1 || $posts->currentPage() > 1) && (Gate::allows('deletePosts', $thread) || Gate::allows('restorePosts', $thread)) && count($selectablePosts) > 0)
            <div class="text-end pb-1">
                <div class="form-check txt-white">
        ...
        @if (! $thread->trashed())
            @can ('reply', $thread)
                <h3 class="text-2xl text-gray-50 pt-3">{{ trans('forum::general.quick_reply') }}</h3>
                <div id="quick-reply" class="pt-3">
